<?php
class HealthDeclarationComponent extends BaseComponent {
	...
	public function createComponentMailForm($name) {
		...
		$form->onSubmit[] = array($this,"mailFormSubmitted");
	...
	public function mailFormSubmitted(Form $form) {
		...
		try {
			$person = $this->getPeople()->findByEmail(strtolower($values["email"]));
		} catch (Exception $ex) {
	...
	public function createComponentDeclaration($name) {
		$person = $this->getPeople()->findByEmail($this->email);
		$personName = null;
		...
		$form->addText("date","Datum")
			->setDisabled()
			->setValue(date("j. n. Y"));		$form->addSubmit("submitted","Potvrzuji a souhlasím");
	...
	public function declarationSubmitted(Form $form) {
		$person = $this->getPeople()->findByEmail(strtolower($this->email));
		$personName = null;
		foreach ($person as $key => $item) {
			if(strpos(strtolower($key), 'jméno a příjmení') !== FALSE) {
				$personName = $item;
		...
		$hd = $this->getHealthDeclaration()->findByEmail(strtolower($this->email));
		if(count($hd) == 1) {
		...
		$template->setFile(APP_DIR . '/templates/mail/health_declaration.phtml');
